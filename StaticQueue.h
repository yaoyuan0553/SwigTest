...
public:
    ...
    inline virtual size_t size() const { return curSize_; }
    inline virtual size_t capacity() const { return maxSize_; }
    inline virtual bool full() const { return curSize_ == maxSize_; }
    inline virtual bool empty() const { return curSize_ == 0; }
    ...
    virtual void emplace_push(T&& item)
    {
        if (full())
            PERROR("trying to push into a full queue");
